...
for LINE in $FILE;do
    ...
    for D in ${DELTA[@]}; do
        # BSA reordering
        ./reordering_benchmark -f ${FILE} -i 0 -z 1 -b ${BLOCK_SIZE} -d ${D}-n ${N_COL} -x 10 -p 1 -m 2 -v 0 -o ${OUTPUT_FILE_BSA}
        # non-reordering
        ./reordering_benchmark -f ${FILE} -i 0 -z 1 -b ${BLOCK_SIZE} -d ${D}-n ${N_COL} -x 10 -p 1 -m 0 -v 0 -o ${OUTPUT_FILE_NON}
    done
done