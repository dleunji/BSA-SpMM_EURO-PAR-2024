...
struct CSR
{
    ...
    // EUNJI
    void read_from_smtx(std::ifstream &infile, std::string delimiter = " ", bool pattern_only = true, bool zero_base = false);
    ...
    CSR(std::ifstream &infile, std::string delimiter = " ", bool pattern_only = true, MatrixFormat mat_fmt = mtx)
    {
        ...
        read_from_smtx(infile, delimiter, pattern_only, true);
    }
    ...
    CSR(CLineReader &cli)
    {
        ...
        read_from_smtx(fin, cli.reader_delimiter_, cli.pattern_only_, true);
        // EUNJI
