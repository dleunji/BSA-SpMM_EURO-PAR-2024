    ...
    if (sparse)
    {
        ...
        if (func)
        {
            ...
            for (int j = 0; j < nonzeros; j++)
            {
                ...
                if (abs((float)output_value_cuda[j] - (float)output_values[j]) > 0.5)
                {
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        if (mixed)
            BmFN<half, half>(benchmark, dimK, vec_length, kernel, alg, sorted, func, sparse);
        else
            BmFN<float, float>(benchmark, dimK, vec_length, kernel, alg, sorted, func, sparse);
    }
}