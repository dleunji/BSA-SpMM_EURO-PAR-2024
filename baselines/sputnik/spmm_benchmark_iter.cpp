...
void BmFN(std::string benchmark, int dimK, int vec_length, int kernel, bool sorted, bool func, int sparse, int iter)
{
    ...
    if (sparse == 1)
    {
        ...
        if (sorted)
        {
            ...
            float ms = 1e3 * (te.tv_sec - ts.tv_sec) + (te.tv_usec - ts.tv_usec) / 1e3;
            std::cout << benchmark << "," << ms << std::endl;
        }
        else
        {
        ...
        {
            printf("Using WMMA \n");
            for (int i = 0;i < iter;i++) spmm::wmmaSpmm(m_vec, vec_length, k, n, d_row_indices, d_row_offsets, d_col_indices, d_value, d_rhs_matrix, d_output_value);
        }
        else if (kernel == 1)
        {
            printf("Using CUDA \n");
            for (int i = 0;i < iter;i++) spmm::cudaSpmm(m_vec, vec_length, k, n, d_row_indices, d_row_offsets, d_col_indices, d_value, d_rhs_matrix, d_output_value);
        }
        else if (kernel == 2)
        {
            ...
            for (int i = 0;i < iter;i++) sputnik::CudaSpmm(m, n, k, nonzeros, d_row_indices, d_value_vec, d_row_offsets, d_col_indices_sputnik_vec, d_rhs_matrix_vec, d_output_value_vec, 0);
        }
        else if (kernel == 3)
        {
...
int main(int argc, char **argv)
{
    ...
    if (argc == 2 && (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0))
    {
        ...
        printf("iter    :   the number of iterations; \n");
    }
    // Run the benchmark
    else
    {
        ...
        int iter = std::atoi(argv[9]);

        if (mixed)
            BmFN<half, half, short, half2, short2, CUDA_R_16F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse, iter);
        else
            BmFN<float, float, int, float, int, CUDA_R_32F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse, iter);
    }
}