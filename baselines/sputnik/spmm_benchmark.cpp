...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        if (mixed)
            BmFN<half, half, short, half2, short2, CUDA_R_16F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse);
        else
            BmFN<float, float, int, float, int, CUDA_R_32F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse);
    }
}