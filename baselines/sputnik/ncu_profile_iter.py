...
parser.add_argument('--bm', default='dlmc/rn50/random_pruning/0.8/bottleneck_2_block_group3_5_1.smtx',
...
parser.add_argument('--iter', type=int, default=1, help='the number of iterations')
...
# If the job is SpMM
if args.job == 'spmm':
    exe_cmd = f'{PRJ_DIR}/baselines/sputnik/spmm_benchmark_iter %s %d %d %d %d %d %d %d %d' % (args.bm, args.dimK, args.dimV, ker, sort, func, sparse, mixed, args.iter)

    if args.prof:
        if args.mem:
            output_file = './csv/mem/spmm/%s_k%d_v%d.csv' % (args.bm.replace('', '').replace('.smtx', '') + suffix, args.dimK, args.dimV)
            exe_cmd = '--metrics l1tex__m_xbar2l1tex_read_bytes_mem_lg_op_ld.sum %s' % exe_cmd
        else:
            output_file = './csv/spmm/%s_k%d_v%d.csv' % (args.bm.replace('', '').replace('.smtx', '') + suffix, args.dimK, args.dimV)
        
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
        cmd = '%s -f --csv --cache-control all --profile-from-start 0 --target-processes all --log-file %s %s' % (args.ncu, output_file, exe_cmd)
    else:
        cmd = exe_cmd
...
# Else if the job is SDDMM
    ...
    exe_cmd = f'{PRJ_DIR}/baselines/sputnik/sddmm_benchmark %s %d %d %d %d %d %d %d %d' % (args.bm, args.dimK, args.dimV, ker, alg, sort, func, sparse, mixed)

    if args.prof:
        output_file = './csv/sddmm/%s_k%d_v%d.csv' % (args.bm.replace('', '').replace('.smtx', '') + suffix, args.dimK, args.dimV)
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
